...
A full code audit identified **30 bugs and 19 architectural violations**, including two cases where entire features never worked correctly.

---

## Current State — Full Problem List

### Critical Bugs — Features That Never Worked

| ID | Problem | Location |
|----|---------|----------|
| B-01 | `save_graph_to_json` writes key `"edges"`, Visualiser reads key `"graph"` → always an empty dict. The sailing graph is **never drawn** | `grib.py:424`, `Visualiser.py:186` |
| B-02 | `save_graph_to_json` is never called anywhere — `sailing_graph.json` is never written | `grib.py:419` |
| B-08 | `save_to_gpx` writes `<trkpt>` elements, but `load_area_file` in the Visualiser reads `gpx.waypoints` (`<wpt>`). Forbidden and caution zones are **never displayed** — always zero polygons | `grib.py:500`, `Visualiser.py:247` |

### Serious Bugs — Crashes and Silent Logic Errors

| ID | Problem | Location |
|----|---------|----------|
| B-18 | `grbs.close()` inside `try` with no `finally` — file handle leak on exception | `load_grib_to_memory` |
| B-19 | `analyze_grib_performance` opens a GRIB handle with no `finally` — same class as B-18 | lines 507–512 |
| B-20 | `curr_lon` corrected by `cos(new curr_lat)` instead of `cos(old curr_lat)` — position calculated with wrong latitude | `simulate_vmg_route:486` |
| B-21 | `calculate_distance_nm` uses a flat-earth formula instead of Haversine — error grows to 3–8% on distances over 200 nm | `calculate_distance_nm` |
| B-22 | No fallback key for U-component (V has a lowercase/uppercase heuristic, U does not) — crash on non-standard GRIB parameter names | `get_weather_from_cache:91` |
| B-23 | Return value of `get_weather_from_cache` is not checked for `None` before unpacking in `generate_reachable_graph` | line 200 |
| B-14 | Same as B-23 in `find_shortest_path_dijkstra_3d` | line 319 |
| B-24 | `distances` and `predecessors` pre-seeded only from `adjacency_map` keys — leaf nodes that appear only as edge targets raise `KeyError` during relaxation | `find_shortest_path_dijkstra:247` |
| B-25 | Timestamps on the Dijkstra 2D path distributed linearly (`idx/len * cost`) instead of accumulated from edge costs — every timestamp in the GPX and log is wrong | `__main__:545–546` |
| B-04 | VMG loop condition `while curr_lat < target_lat` hard-codes a northward destination — empty route or infinite loop for any other direction | `simulate_vmg_route:446` |
| B-26 | `{time_hours:.2f}` in `print_route_summary` crashes when `time_hours` remains `None` | line 417 |
| B-27 | The "Filter land" checkbox calls `check_files_loop(force=True)` — the `root.after` reschedule is skipped — **auto-refresh stops permanently** after one click | `Visualiser.py:267,321` |

### Code Duplication

| Problem | Copies |
|---------|--------|
| `tws = sqrt(u²+v²) * 1.94384` + `twd = atan2(-u,-v)` | 5× |
| Key heuristic `'10 metre v/V wind component'` | 3× |
| `polars = get_scampi_30_polars()` called inside routing functions | 4× |

### API and Structural Problems

| Problem | Consequence |
|---------|-------------|
| `generate_reachable_graph` always writes `graph_log.txt` as a side effect | Cannot use the function without disk I/O; makes testing harder |
| Polars hard-coded inside routing functions | Cannot change the yacht without editing every function |
| `print(f"[DIAG]...")` instead of the `logging` module | Cannot be silenced, redirected, or filtered by level |
| `__main__` with hard-coded coordinates, filenames, and simulation count | Cannot run with different parameters without editing the source |
| All files (source, GRIB data, GPX, logs, JSON) in one directory | No separation between source code, data, and generated artefacts |
| `check_files_loop` in the Visualiser has three identical loops (VMG / Fastest / 3D) | Any logic change must be made in three places |
| GPX filenames hard-coded in the Visualiser | Must be manually synchronised with `grib.py` |

### Minor Issues in Visualiser.py

| ID | Problem |
|----|---------|
| B-28 | `manual_load_gpx` permanently overwrites `vmg_files[0]` — auto-monitoring for VMG slot 1 is broken until restart |
| B-29 | Area shapes not cleared when source GPX file disappears — ghost forbidden zones remain on the map |
| B-30 | `np.empty(..., dtype=object)` for `first_time_grid` — uninitialised cells; `np.full(..., None)` is safer |
| — | OpenSeaMap set as standalone base layer instead of overlay — nearly blank map |
| — | `draw_global_land_mask` blocks the UI thread; triggered on every file update |
| — | Bounding box only ever grows — old bounds never reset on force reload |
| — | `is_square_land` ignores the `delta` parameter — land filter checks only cell centre |
| — | `force=True` re-centres the map for every `index==0` regardless of `type_key` — map jumps around |
...
The project already has tests, CI, and documentation — it is not a proof-of-concept. Cost: one `pyproject.toml` and an `__init__.py`. Benefits: clean inter-module imports, `pip install -e .`, a named CLI entrypoint, and straightforward support for a second yacht in the future. Flat layout (all files in the root) produces messy imports and requires `sys.path` hacks in tests — something that already exists in the current test file.
