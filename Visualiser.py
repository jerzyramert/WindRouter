...
class GPXViewerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Scampi Navigation - Multi-Route Monitor Full")
    ...
    def setup_ui(self):
        ...
        # 1. Status panel for 8 routes
        self.status_frame = tk.LabelFrame(self.top_frame, text="Route Status (VMG & Fastest)")
        ...
        # Default view (Northern Europe/Baltic)
    ...
    def update_bounds(self, lat, lon):
        """Update boundaries for land mask generation."""
    ...
    def is_square_land(self, lat, lon, delta):
        """Checks land only at the center of the square (fast)."""
        if not HAS_LAND_MASK: return False
        return globe.is_land(lat, lon)

    def draw_global_land_mask(self):
        """Draws grey squares where land is present in the operational area."""
    ...
    def load_track(self, file_path, index, is_vmg, force=False):
        ...
        try:
            ...
            if pts:
                if is_vmg:
                    ...
                    self.lbl_fastest[index].config(text=f"Fastest {index+1}: {len(pts)} pts")
    ...
    def load_area_file(self, file_path, shapes_list, fill_color, outline_color, info_label, prefix):
        """Loads area markers (rectangles) from GPX file."""
        if not os.path.exists(file_path): return
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                gpx = gpxpy.parse(f)
    ...
    def force_reload_areas(self):
        """Resets mtimes and forces a complete redraw of all elements."""
        self.last_vmg_mtimes = [0]*4
        self.last_fastest_mtimes = [0]*4
        self.last_forbidden_mtime = 0
        self.last_not_recommended_mtime = 0
    ...
    def check_files_loop(self, force=False):
        """Main loop monitoring 11 files for changes."""
        any_update = False
        ...
        for i, path in enumerate(self.vmg_files):
            if os.path.exists(path):
                ...
                if m > self.last_vmg_mtimes[i] or force:
                    self.load_track(path, i, True, force)
        ...
        for i, path in enumerate(self.fastest_files):
            if os.path.exists(path):
                ...
                if m > self.last_fastest_mtimes[i] or force:
                    self.load_track(path, i, False, force)
        ...
        if os.path.exists(self.graph_file):
            ...
            if m > self.last_graph_mtime or force:
                self.load_sailing_graph()
                self.last_graph_mtime = m
                any_update = True
        ...
        if os.path.exists(self.not_recommended_file):
            ...
            if m > self.last_not_recommended_mtime or force:
                ...
                self.last_not_recommended_mtime = m
                any_update = True

        # Update land mask only if any data has changed
    ...
    def manual_load_gpx(self):
        ...
        if f:
            # Manual load replaces VMG 1 for preview purposes
            self.vmg_files[0] = f
            self.last_vmg_mtimes[0] = 0
            self.load_track(f, 0, True, True)
